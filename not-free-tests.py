...
class TestYourWebserver(unittest.TestCase):
    ...
    def test_get_404(self):
        ...
        try:
            ...
            self.assertTrue(e.getcode() == 404,
                            ("404 Not FOUND! %d" % e.getcode()))
        else:
    ...
    def test_get_group(self):
        ...
        try:
            req = request.urlopen(url, None, 3)
            self.assertTrue(
                False, "Should have thrown an HTTP Error! [%d]" % req.getcode())
        except request.HTTPError as e:
            self.assertTrue(e.getcode() == 404,
                            ("404 Not FOUND! %d" % e.getcode()))
        else:
    ...
    def test_405(self):
        ...
        try:
            ...
            self.assertTrue(req.getcode() == 405,
                            ("405 Not FOUND! %d" % req.getcode()))
            self.assertTrue(
                False, "Should have thrown an HTTP 405 Error for /deep.css!")
        except request.HTTPError as e:
            self.assertTrue(e.getcode() == 405,
                            ("405 Not FOUND! %d" % e.getcode()))
    ...
    def test_deep_no_end(self):
        ...
        try:
            ...
            if code >= 200 and code <= 299 and req.geturl() == expected_url:
                ...
                self.assertTrue(
                    False, "The URL hasn't changed %s %s" % (code, req.geturl()))
        except request.HTTPError as e:
            code = e.getcode()
            self.assertTrue(code >= 300 and code < 400,
                            "300ish Not FOUND! %s" % code)
    ...
    def test_hardcode(self):
        ...
        req = request.urlopen(url, None, 3)
        self.assertTrue(req.getcode() == 200,
                        "200 OK Not FOUND! Hardcoding? /hardcode/index.html")
        self.assertTrue(req.info().get_content_type() == "text/html",
                        ("Bad mimetype for html! %s" % req.info().get_content_type()))
        url = self.baseurl + "/hardcode/"
        req = request.urlopen(url, None, 3)
        self.assertTrue(req.getcode() == 200,
                        "200 OK Not FOUND! Hardcoding? /hardcode/")
        self.assertTrue(req.info().get_content_type() == "text/html",
                        ("Bad mimetype for html! %s" % req.info().get_content_type()))
    ...
    def test_hardcode2(self):
        ...
        try:
            req = request.urlopen(url, None, 3)
            self.assertTrue(
                False, "Should have thrown an HTTP Error for /deep.css!")
        except request.HTTPError as e:
            self.assertTrue(e.getcode() == 404,
                            ("404 Not FOUND! %d" % e.getcode()))
        else:
        ...
        try:
            req = request.urlopen(url, None, 3)
            self.assertTrue(
                False, "Should have thrown an HTTP Error for /deep/deep!")
        except request.HTTPError as e:
            self.assertTrue(e.getcode() == 404,
                            ("404 Not FOUND! %d" % e.getcode()))
        else:
